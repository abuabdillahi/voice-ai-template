...
- HIPAA compliance, BAAs with vendors, PHI handling. The MVP collects no PHI by design — the persisted `messages` and `safety_events` rows contain symptom descriptions but no identifiers beyond the Supabase `user_id`. Productionising for a US clinical context requires a separate compliance workstream.
